...
\[ \Sigma_{ij} = \mathrm{clip}\left( \frac{\log(x_{ij} + \epsilon) - \mathrm{median}_j}{\mathrm{IQR}_j}, -c, c \right) \]
...
\[ \Sigma_{ij} = \mathrm{clip}\left( \log \left( \frac{A_{ij} + \epsilon}{E_{ij} + \epsilon} \right), -c, c \right) \]
